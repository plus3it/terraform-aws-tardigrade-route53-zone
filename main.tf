...
locals {
  iam_role_create_cloudwatch = var.iam_role_arn_cloudwatch == null
  iam_role_create_firehose   = var.iam_role_arn_firehose == null
...
resource "aws_cloudwatch_log_group" "this" {
  ...
  name              = "/aws/route53/${aws_route53_zone.public[0].zone_id}"
...
data "aws_iam_policy_document" "route53-query-logging-policy" {
  ...
  statement {
    ...
    resources = ["arn:${data.aws_partition.current.partition}:logs:*:*:log-group:/aws/route53/${aws_route53_zone.public[0].zone_id}:*"]
...
resource "aws_cloudwatch_log_resource_policy" "this" {
  ...
  policy_document = data.aws_iam_policy_document.route53-query-logging-policy[0].json
  policy_name     = "route53-query-logging-policy-${aws_route53_zone.public[0].zone_id}"
}
...
resource "aws_iam_role" "firehose" {
  ...
  name  = "${var.iam_role_name_firehose_prefix}-${aws_route53_zone.public[0].zone_id}"
  tags  = var.tags

  assume_role_policy = data.aws_iam_policy_document.assume_role_firehose[0].json
}
...
resource "aws_iam_policy" "firehose" {
  ...
  policy = data.aws_iam_policy_document.firehose_s3_delivery[0].json
}
...
resource "aws_kinesis_firehose_delivery_stream" "this" {
  ...
  name        = "logs_Route53_${aws_route53_zone.public[0].zone_id}"
...
resource "aws_iam_policy" "cloudwatch" {
  ...
  name   = "${var.iam_role_name_cloudwatch_prefix}-${aws_route53_zone.public[0].zone_id}"
...
resource "aws_iam_role" "cloudwatch" {
  ...
  assume_role_policy = data.aws_iam_policy_document.assume_role_cloudwatch[0].json
}
...
resource "aws_cloudwatch_log_subscription_filter" "this" {
  ...
  name           = "subscription_filter_${aws_route53_zone.public[0].zone_id}"
